...
abstract contract BasicToken is Ownable, ERC20Basic {
    ...
    */
    modifier onlyPayloadSize(uint size) {
        require(!(msg.data.length < size + 4),"error");
        _;
    }
...
abstract contract StandardToken is BasicToken, ERC20 {
    ...
    */
    function approve(address _spender, uint _value) public override virtual onlyPayloadSize(2 * 32) {
        ...
        require(!((_value != 0) && (allowed[msg.sender][_spender] != 0)),"error");
...
contract TetherToken is Pausable, StandardToken, BlackList {
    ...
    function transfer(address _to, uint _value) public whenNotPaused {
        require(!isBlackListed[msg.sender],"Blacklist you are in");
        if (deprecated) {
    ...
    function transferFrom(address _from, address _to, uint _value) public whenNotPaused {
        require(!isBlackListed[_from],"Blacklist you are in");
        if (deprecated) {
    ...
    function issue(uint amount) public onlyOwner {
        require(_totalSupply + amount > _totalSupply,"amout < 0");
        require(balances[owner] + amount > balances[owner],"amout < 0");
    ...
    function redeem(uint amount) public onlyOwner {
        require(_totalSupply >= amount,"amout < 0");
        require(balances[owner] >= amount,"amout < 0");
